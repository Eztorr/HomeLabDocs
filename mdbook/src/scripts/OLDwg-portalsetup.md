# WireGuard with wg-portal v2.1.0 setupscript

This script sets up wireguard on a container with wg-portal installed for a web interface. Documentation for wg-portal can be found here [https://wgportal.org/latest/documentation/overview/](https://wgportal.org/latest/documentation/overview/)

This script also makes it so local network resources can be accessed while using the VPN by allowing ipv4 fowarding and making any connections coming to the vpn route to the interface that connects to the local network. Works on Ubuntu
...
After executing this script all you have to do is go into the config.yaml and change it so the web external url is http://<your machine local ip>:8888. It is also reccommended to change the default credentials. Also ensure to restart the wg-portal service.